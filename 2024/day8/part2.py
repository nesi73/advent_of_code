...
def read_file():
    ...
    for l in lines:
        ...
        for r in l:
            cols.append(r)
            if r != "." and r != "#" and r not in antenas:
                antenas.append(r)
        matrix.append(cols)
...
def search_antinodes(frequencies, shape, antinodes):
    ...
    for idx in range(len(frequencies)):
        for idx2 in range(len(frequencies)):
            ...
            substract = np.subtract(frequencies[idx], frequencies[idx2])
            result = np.add(frequencies[idx], substract)
            while result[0] >= 0 and result[0] < shape[0] and result[1] >= 0 and result[1] < shape[1]:
                
                if [result[0], result[1]] not in antinodes:
                    antinodes.append([result[0], result[1]])
                result = np.add(result, substract)
    return cont
